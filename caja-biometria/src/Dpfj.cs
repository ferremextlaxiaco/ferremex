...
namespace Ferremex.Biometria
{
    ...
    public static class Dpfj
    {
        ...
        {
            EnsureInit();
            int rc = dpfj_start_enrollment(DPFJ_FMD_ANSI_378_2004);
            if (rc != 0) throw new DpException("dpfj_start_enrollment falló", rc);
            try
            {
                int mejorCalidad = 0;
                for (int i = 1; i <= muestras; i++)
                {
                    ...
                    if (onEsperando != null) onEsperando(i, muestras, 0);
                    var m = CapturarMuestra(timeoutMsPorMuestra);
                    rc = dpfj_add_to_enrollment(DPFJ_FMD_ANSI_378_2004, m.Fmd, (uint)m.Fmd.Length, 0);
                    if (rc != 0) throw new DpException("dpfj_add_to_enrollment falló", rc);
                    if (m.Calidad > mejorCalidad) mejorCalidad = m.Calidad;
                    if (onMuestraOk != null) onMuestraOk(i, muestras, m.Calidad);
                }
                var fmd = new byte[FMD_BUF];
                uint fmdSize = FMD_BUF;
                rc = dpfj_create_enrollment_fmd(fmd, ref fmdSize);
                if (rc != 0) throw new DpException("dpfj_create_enrollment_fmd falló (muestras inconsistentes)", rc);
                var outFmd = new byte[fmdSize];
                Array.Copy(fmd, outFmd, (int)fmdSize);
                return new Muestra { Fmd = outFmd, Calidad = mejorCalidad };
            }
            finally { dpfj_finish_enrollment(); }
        }
    }
}
